...
export const createTransaction = async (req, res, next) => {
  try {
    ...
    if (!title || !amount || !category || !categoryType) {
      return res.status(400).json({ message: 'Title, amount, category and categoryType are required.' });
    }
...
export const deleteTransection = async (req, res, next) => {
  try {
    ...
    console.log('User:', req.user); // Should show _id
